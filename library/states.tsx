export const allStates = [
  {
    state: "Abia",
    capital: "Umuahia",
    slogan: "God's Own State",
    senatorial_districts: ["Abia Central", "Abia North", "Abia South"],
    lgas: [
      "Aba North",
      "Aba South",
      "Arochukwu",
      "Bende",
      "Isuikwuato",
      "Osisioma-Ngwa",
      "Obioma-Ngwa",
      "Ohafia",
      "Ikwuano",
      "Umu-Nneochi",
      "Isiala Ngwa South",
      "Isiala Ngwa North",
      "Ugwunagbo",
      "Ukwa West",
      "Ukwa East",
      "Umuahia South",
      "Umuahia North",
    ],
    landmass: "6,320 km2 (2,440 sq mi)",
    population: "2,833,999",
    dialect: "Igbo",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Abia_state_map.png/250px-Nigeria_Abia_state_map.png",
    latitude: "5.524913",
    longitude: "7.494296",
    website: "http://www.abiastate.gov.ng/",
    geo_politcal_zone: "South East",
    created_date: "27 August 1991",
    created_by: "General Ibrahim Babangida",
    past_governors: [
      "Ogbonnaya Onu",
      "Chris Akomas",
      "Orji Uzor Kalu",
      "Theodore Orji",
      "Okezie Ikpeazu",
    ],
    borders: ["Akwa Ibom", "Cross River", "Ebonyi", "Enugu", "Imo", "Rivers"],
    known_for: [
      "Enyimba International F.C.",
      "Abia Warriors F.C.",
      "Long Juju Slave Route, Arochukwu",
      "Michael Okpara University of Agriculture",
      "National War Museum",
      "Azumini Blue River",
    ],
  },
  {
    state: "Adamawa",
    capital: "Yola",
    slogan: "Land of Beauty, Sunshine and Hospitality",
    senatorial_districts: ["Adamawa Central", "Adamawa North", "Adamawa South"],
    lgas: [
      "Demsa",
      "Fufore",
      "Ganye",
      "Girei",
      "Gombi",
      "Jada",
      "Yola North",
      "Lamurde",
      "Madagali",
      "Maiha",
      "Mayo-Belwa",
      "Michika",
      "Mubi South",
      "Mubi North",
      "Numna",
      "Shelleng",
      "Song",
      "Toungo",
      "Jimeta",
      "Yola South",
      "Hung",
    ],
    landmass: "36,917 km2 (14,254 sq mi)",
    population: "3,168,101",
    dialect: "Hausa",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Adamawa_state_map.png/250px-Nigeria_Adamawa_state_map.png",
    latitude: "9.2094",
    longitude: "12.4818",
    website: "http://www.adamawastate.gov.ng/",
    geo_politcal_zone: "North East",
    created_date: "27 August 1991",
    created_by: "General Ibrahim Babangida",
    past_governors: [
      "Abubakar Saleh Michika",
      "Abdulrahman Adamu",
      "Ahmadu Joda",
      "Aliyu Mohammed",
      "Alhaji Abubakar Barde",
      "Alhaji Bamanga Tukur",
      "Alhaji Saleh Michika",
      "Alhaji Umaru Fintiri",
    ],
    borders: ["Borno", "Gombe", "Taraba", "Cameroon", "Bauchi"],
    known_for: [
      "Yola Airport",
      "American University of Nigeria",
      "Modibbo Adama University of Technology",
      "Adamawa State Polytechnic",
      "Adamawa State University",
      "Adamawa State College of Nursing and Midwifery",
      "Adamawa State College of Agriculture",
      "Adamawa State College of Legal and Islamic Studies",
      "Adamawa State College of Education",
      "Adamawa State College of Basic and Remedial Studies",
      "Adamawa State College of Arts and Science",
    ],
  },
  {
    state: "Akwa Ibom",
    capital: "Uyo",
    slogan: "Land of Promise",
    senatorial_districts: [
      "Akwa Ibom North East",
      "Akwa Ibom North West",
      "Akwa Ibom South",
    ],
    lgas: [
      "Abak",
      "Eastern Obolo",
      "Eket",
      "Essien Udim",
      "Etimekpo",
      "Etinan",
      "Ibeno",
      "Ibesikpo Asutan",
      "Ibiono Ibom",
      "Ika",
      "Ikono",
      "Ikot Abasi",
      "Ikot Ekpene",
      "Ini",
      "Itu",
      "Mbo",
      "Mkpat Enin",
      "Nsit Ibom",
      "Nsit Ubium",
      "Obot Akara",
      "Okobo",
      "Onna",
      "Orukanam",
      "Oron",
      "Udung Uko",
      "Ukanafun",
      "Esit Eket",
      "Uruan",
      "Urue Offoung",
      "Oruko Ete",
      "Uyo",
    ],
    landmass: "7,081 km2 (2,735 sq mi)",
    population: "5,450,758",
    dialect: "Ibibio",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "4.9833",
    longitude: "7.9167",
    website: "http://www.akwaibomstate.gov.ng/",
    geo_politcal_zone: "South South",
    created_date: "23 September 1987",
    created_by: "General Ibrahim Babangida",
    past_governors: [
      "Idongesit Nkanga",
      "Victor Attah",
      "Godswill Akpabio",
      "Udom Gabriel Emmanuel",
    ],
    borders: ["Cross River", "Abia", "Rivers", "Bayelsa"],
    known_for: [
      "Bridge of No Return, Ikot Abasi",
      "Amalgamtion House",
      "Ibibio Museu and State Park",
      "National Museum of Colonial History",
    ],
  },
  {
    state: "Anambra",
    capital: "Awka",
    slogan: "Light of the Nation",
    senatorial_districts: ["Anambra Central", "Anambra North", "Anambra South"],
    lgas: [
      "Aguata",
      "Akwa North",
      "Anambra",
      "Anambra-West",
      "Anaocha",
      "Awka-North",
      "Awka-South",
      "Ayamelum",
      "Dunukofia",
      "Ekwusigo",
      "Idemili-North",
      "Idemili-South",
      "Ihiala",
      "Imo",
      "Nibo",
      "Njikoka",
      "Nnewi-North",
      "Nnewi-South",
      "Ogbaru",
      "Olumba",
      "Onitsha-North",
      "Onitsha-South",
      "Orumba-North",
      "Orumba-South",
      "Oti",
      "Otu-Ocha",
      "Ubuluizor Ihiala",
      "Uyi",
    ],
    landmass: "4,844 km2 (1,870 sq mi)",
    population: "4,182,032",
    dialect: "Igbo",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "6.2107",
    longitude: "7.0323",
    website: "http://www.anambrastate.gov.ng/",
    geo_politcal_zone: "South East",
    created_date: "27 August 1991",
    created_by: "General Ibrahim Babangida",
    past_governors: [
      "Chukwuemeka Ezeife",
      "Mike Attah",
      "Chinwoke Mbadinuju",
      "Chris Ngige",
      "Peter Obi",
      "Virginia Etiaba",
      "Andy Uba",
      "Chukwuemeka Ezeife",
      "Chris Ngige",
      "Peter Obi",
      "Willie Obiano",
    ],
    borders: ["Kogi", "Delta", "Abia", "Enugu", "Imo"],
    known_for: [
      "Ogbunike Caves",
      "Agulu Lake",
      "Ogbaukwu Caves and Waterfalls",
    ],
  },
  {
    state: "Bauchi",
    capital: "Bauchi",
    slogan: "Pearl of Tourism",
    senatorial_districts: ["Bauchi Central", "Bauchi North", "Bauchi South"],
    lgas: [
      "Alkaleri",
      "Bauchi",
      "Bogoro",
      "Damban",
      "Darazo",
      "Dass",
      "Gamawa",
      "Ganjuwa",
      "Giade",
      "Itas/Gadau",
      "Jama'Are",
      "Katagum",
      "Kirfi",
      "Misau",
      "Ningi",
      "Shira",
      "Tafawa-Balewa",
      "Toro",
      "Warji",
      "Zaki",
    ],
    landmass: "49,119 km2 (18,962 sq mi)",
    population: "4,653,066",
    dialect: "Hausa",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "10.3158",
    longitude: "9.8442",
    website: "http://www.bauchistate.gov.ng/",
    geo_politcal_zone: "North East",
    created_date: "3 February 1976",
    created_by: "General Murtala Mohammed",
    past_governors: [
      "Muhammadu Buhari",
      "Abdulkadir Ahmed",
      "Abdullahi Sarki Mukhtar",
      "Ahmed Adamu Mu'azu",
      "Adamu Mu'azu",
      "Isa Yuguda",
      "Muhammad Abdullahi Abubakar",
    ],
    borders: ["Kano", "Jigawa", "Plateau", "Taraba", "Gombe", "Yobe"],
    known_for: [
      "Yankari National Park",
      "Sumu Wildlife Reserve",
      "Wiki Warm Springs",
      "Emir's Palace",
      "Bauchi Central Mosque",
      "Bauchi State Library",
    ],
  },
  {
    state: "Benue",
    capital: "Makurdi",
    slogan: "Food Basket of the Nation",
    senatorial_districts: [
      "Benue North East",
      "Benue North West",
      "Benue South",
    ],
    lgas: [
      "Ado",
      "Agatu",
      "Apa",
      "Buruku",
      "Gboko",
      "Guma",
      "Gwer-East",
      "Gwer-West",
      "Katsina-Ala",
      "Konshisha",
      "Kwande",
      "Logo",
      "Makurdi",
      "Ogbadibo",
      "Ohimini",
      "Oju",
      "Okpokwu",
      "Otukpo",
      "Oturkpa",
      "Tarka",
      "Ukum",
      "Vandekya",
    ],
    landmass: "34,059 km2 (13,139 sq mi)",
    population: "4,253,641",
    dialect: "Tiv",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "7.7327",
    longitude: "8.5214",
    website: "http://www.benuestate.gov.ng/",
    geo_politcal_zone: "North Central",
    created_date: "3 February 1976",
    created_by: "General Murtala Mohammed",
    past_governors: [
      "Aper Aku",
      "Yohanna Madaki",
      "Apiafi Ordue",
      "Joshua Obademi",
      "John Atom Kpera",
      "Idris Garba",
      "George Akume",
      "Gabriel Suswam",
      "Samuel Ortom",
    ],
    borders: ["Taraba", "Nasarawa", "Cross River", "Enugu", "Kogi"],
    known_for: [
      "Benue River",
      "Katsina-Ala River",
      "Kwande River",
      "Katsina-Ala River",
      "Ushongo Hills",
      "Benue State University",
      "Federal University of Agriculture",
      "Benue State Polytechnic",
      "College of Education",
      "Benue State University Teaching Hospital",
      "Federal Medical Center",
      "Benue State Library",
    ],
  },
  {
    state: "Borno",
    capital: "Maiduguri",
    slogan: "Home of Peace",
    senatorial_districts: ["Borno Central", "Borno North", "Borno South"],
    lgas: [
      "Abadan",
      "Askira-Uba",
      "Bama",
      "Bayo",
      "Biu",
      "Chibok",
      "Damboa",
      "Dikwa",
      "Gubio",
      "Guzamala",
      "Gwoza",
      "Hawul",
      "Jere",
      "Kaga",
      "Kala/Balge",
      "Konduga",
      "Kukawa",
      "Kwaya-Kusar",
      "Mafa",
      "Magumeri",
      "Maiduguri",
      "Marte",
      "Mobbar",
      "Mongunu",
      "Ngala",
      "Nganzai",
      "Shani",
    ],
    landmass: "70,898 km2 (27,357 sq mi)",
    population: "4,171,104",
    dialect: "Kanuri",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "11.8333",
    longitude: "13.1500",
    website: "http://www.bornostate.gov.ng/",
    geo_politcal_zone: "North East",
    created_date: "3 February 1976",
    created_by: "General Murtala Mohammed",
    past_governors: [
      "Muhammed Goni",
      "Abubakar Waziri",
      "Abdulkadir Kure",
      "Ali Modu Sheriff",
      "Kashim Shettima",
    ],
    borders: ["Yobe", "Adamawa", "Gombe", "Bauchi", "Yobe", "Cameroon", "Chad"],
    known_for: [
      "Kanuri people",
      "Kanem-Bornu Empire",
      "Kanem Empire",
      "Kanem-Bornu Empire",
    ],
  },
  {
    state: "Bayelsa",
    capital: "Yenagoa",
    slogan: "Glory of all Lands",
    senatorial_districts: ["Bayelsa Central", "Bayelsa East", "Bayelsa West"],
    lgas: [
      "Adagbabiri",
      "Brass",
      "Ekeremor",
      "Kembe",
      "Kolokuma",
      "Kolokuma/Opkuma",
      "Nembe",
      "Ogbia",
      "Sagbama",
      "Southern-Ijaw",
      "Toru-Abubo",
      "Yenegoa",
    ],
    landmass: "10,773 km2 (4,158 sq mi)",
    population: "1,704,515",
    dialect: "Ijaw",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "4.9267",
    longitude: "6.2676",
    website: "http://www.bayelsa.gov.ng/",
    geo_politcal_zone: "South South",
    created_date: "1 October 1996",
    created_by: "General Sani Abacha",
    past_governors: [
      "Diepreye Alamieyeseigha",
      "Goodluck Jonathan",
      "Timipre Sylva",
      "Seriake Dickson",
    ],
    borders: ["Rivers", "Delta"],
    known_for: [
      "The Ox-Bow Lake, Yenagoa",
      "Oloibiri Oil Museum, Oloibiri",
      "Isaac Boro Garden Park, Yenagoa",
      "Peace Park, Yenagoa",
      "Akassa Wildlife Forest, Akassa",
      "Ogboinbiri Oil Well, Ogboinbiri",
    ],
  },
  {
    state: "Cross River",
    capital: "Calabar",
    slogan: "The People's Paradise",
    senatorial_districts: [
      "Cross River Central",
      "Cross River North",
      "Cross River South",
    ],
    lgas: [
      "Abi",
      "Abuochichie",
      "Akamkpa",
      "Akpabuyo",
      "Bakassi",
      "Bekwara",
      "Biasi",
      "Boki",
      "Calabar-Municipal",
      "Calabar-South",
      "Etunk",
      "Ikom",
      "Obanliku",
      "Obubra",
      "Obudu",
      "Odukpani",
      "Ogoja",
      "Ugep-North",
      "Yakurr",
      "Yala",
    ],
    landmass: "20,156 km2 (7,781 sq mi)",
    population: "2,888,966",
    dialect: "Efik",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "5.6367",
    longitude: "8.3333",
    website: "http://www.crossriverstate.gov.ng/",
    geo_politcal_zone: "South South",
    created_date: "27 May 1967",
    created_by: "General Yakubu Gowon",
    past_governors: [
      "Clement Isong",
      "Donald Etiebet",
      "Daniel Archibong",
      "Paul Omu",
      "Ernest Atta",
      "Gregory Agboneni",
      "Umar Faoruk Ahmed",
      "Christopher Osondu",
      "Donald Duke",
      "Liyel Imoke",
      "Benedict Ayade",
    ],
    borders: ["Benue", "Ebonyi", "Enugu", "Benue", "Kogi", "Akwa Ibom"],
    known_for: [
      "Obudu Cattle Ranch",
      "Obudu Mountain Resort",
      "Kwa Falls",
      "Agbokim Waterfalls",
      "Tinapa Resort",
      "Cross River National Park",
      "Cross River Gorilla",
      "Cross River State National Park",
      "Cross River State National Park",
    ],
  },
  {
    state: "Delta",
    capital: "Asaba",
    slogan: "The Big Heart",
    senatorial_districts: ["Delta Central", "Delta North", "Delta South"],
    lgas: [
      "Aniocha North",
      "Aniocha-North",
      "Aniocha-South",
      "Bomadi",
      "Burutu",
      "Effurun",
      "Ethiope-East",
      "Ethiope-West",
      "Idu",
      "Ika-Ne",
      "Ika-North-East",
      "Ika-South",
      "Ikpemili",
      "Isoko-North",
      "Isoko-South",
      "Ndokwa-East",
      "Ndokwa-North",
      "Ndokwa-South",
      "Ndokwa-West",
      "Okpe",
      "Okwuani",
      "Oleh",
      "Oshielli-North",
      "Oshimili",
      "Oshimili-North",
      "Oshimili-South",
      "Osimili",
      "Osimili-North",
      "Osimili-South",
      "Patani",
      "Sapele",
      "Udokwa",
      "Udu",
      "Ughelli-North",
      "Ughelli-South",
      "Ukwuani",
      "Uraun",
      "Urwie",
      "Uvie",
      "Uvwei",
      "Uvwie",
      "Uwvie",
      "Warri-Central",
      "Warri-North",
      "Warri-South",
    ],
    landmass: "17,698 km2 (6,826 sq mi)",
    population: "4,112,445",
    dialect: "Igbo",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "5.6367",
    longitude: "8.3333",
    website: "http://www.deltastate.gov.ng/",
    geo_politcal_zone: "South South",
    created_date: "27 August 1991",
    created_by: "General Ibrahim Babangida",
    past_governors: [
      "Felix Ibru",
      "John Dungs",
      "Bassey Asuquo",
      "Augustine L. Iyasele",
      "Olorogun Felix Ovuodoroye Ibru",
      "James Ibori",
      "Uduaghan",
      "Emmanuel Uduaghan",
      "Arthur Ifeanyi Okowa",
    ],
    borders: ["Edo", "Ondo", "Anambra", "Imo", "Rivers", "Bayelsa"],
    known_for: [
      "Nana of Itsekiri’s palace/Nana Living History Museum, Koko",
      "Ogbeowele Forest Reserve, Abraka",
      "Ogbe-Ijoh Waterside Market, Warri",
      "Ogbeke Square, Asaba",
      "Lander Brothers Anchorage, Asaba",
      "Ogbeogonogo Modern Market, Asaba",
      "Okpuzu Waterfa,ll, Ibusa",
    ],
  },
  {
    state: "Ebonyi",
    capital: "Abakaliki",
    slogan: "Salt of the Nation",
    senatorial_districts: ["Ebonyi South", "Ebonyi Central", "Ebonyi North"],
    lgas: [
      "Abakaliki",
      "Afikpo-North",
      "Afikpo-South",
      "Bomadim",
      "Ebonyi",
      "Ezza-North",
      "Ezza-South",
      "Ikwo",
      "Ishielu",
      "Ivo",
      "Izzi",
      "Obaukwu",
      "Ohakwu",
      "Onicha",
      "Ukaba",
    ],
    landmass: "5,670 km2 (2,190 sq mi)",
    population: "2,176,947",
    dialect: "Igbo",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "6.3167",
    longitude: "8.1000",
    website: "http://www.ebonyistate.gov.ng/",
    geo_politcal_zone: "South East",
    created_date: "1 October 1996",
    created_by: "General Sani Abacha",
    past_governors: ["Sam Egwu", "Martin Elechi", "Dave Umahi"],
    borders: ["Benue", "Cross River", "Enugu", "Imo"],
    known_for: [
      "Mmahi Ezi Salt Lake, Okposi Okwu",
      "Amachor Cave, Amagu",
      "Ndibe Sand Beach, Afikpo",
      "Greater Rice Husks, Abakaliki",
    ],
  },
  {
    state: "Edo",
    capital: "Benin City",
    slogan: "Heartbeat of the Nation",
    senatorial_districts: ["Edo South", "Edo Central", "Edo North"],
    lgas: [
      "Afokpella",
      "Afuze",
      "Agbazilo",
      "Akoko Edo",
      "Akoko-Edo",
      "Egor",
      "Esan-Central",
      "Esan-North-East",
      "Esan-North-East",
      "Esan-South-East",
      "Esan-West",
      "Etsako-Central",
      "Etsako-East",
      "Etsako-West",
      "Igueben",
      "Iguobano North East",
      "Ikpoba-Okha",
      "Ohunmwode",
      "Ologbo",
      "Opoji Irrua",
      "Opoji Irrua",
      "Oredo",
      "Orhionmwon",
      "Ovia-North-East",
      "Ovia-South-West",
      "Owan East",
      "Owan-East",
      "Owan-West",
      "Uhunmwonde",
    ],
    landmass: "17,802 km2 (6,873 sq mi)",
    population: "3,233,366",
    dialect: "Bini",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "6.3167",
    longitude: "8.1000",
    website: "http://www.edostate.gov.ng/",
    geo_politcal_zone: "South South",
    created_date: "27 August 1991",
    created_by: "General Ibrahim Babangida",
    past_governors: [
      "John Odigie Oyegun",
      "Lucky Igbinedion",
      "Adams Oshiomhole",
      "Godwin Obaseki",
    ],
    borders: ["Delta", "Kogi", "Ondo", "Anambra", "Rivers"],
    known_for: [
      "Benin Moat, Benin City",
      "Benin National Museum, Benin City",
      "Emotan Statue, Benin City",
      "Igun Bronze Casters Guild, Benin City",
    ],
  },
  {
    state: "Ekiti",
    capital: "Ado-Ekiti",
    slogan: "Land of Honour",
    senatorial_districts: ["Ekiti South", "Ekiti Central", "Ekiti North"],
    lgas: [
      "Ado-Ekiti",
      "Aiyekire",
      "Efon",
      "Ekiti-East",
      "Ekiti-South-West",
      "Ekiti-West",
      "Emure/Ise/Orun",
      "Gbonyin",
      "Ido-Osi",
      "Ijero",
      "Ikare",
      "Ikere",
      "Ikole",
      "Ilejemeje",
      "Irepodun/Ifelodun",
      "Ise-Orun",
      "Moba",
      "Oye",
    ],
    landmass: "6,353 km2 (2,454 sq mi)",
    population: "2,384,212",
    dialect: "Yoruba",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "7.6667",
    longitude: "5.3333",
    website: "http://www.ekitistate.gov.ng/",
    geo_politcal_zone: "South West",
    created_date: "1 October 1996",
    created_by: "General Sani Abacha",
    past_governors: [
      "Niyi Adebayo",
      "Ayo Fayose",
      "Segun Oni",
      "Kayode Fayemi",
      "Ayo Fayose",
    ],
    borders: ["Kogi", "Kwara", "Ondo", "Osun"],
    known_for: [
      "Ikogosi Warm and Cold Springs, Ikogosi",
      "Olosunta, Ikere",
      "Ori Oke Owa, Ikere",
      "Ori Oke Aiyegunle, Ikere",
      "Arinta Waterfalls, Ipole-Iloro",
      "Okemesi Hills, Okemesi",
      "Olosunta, Ughele and Orole Hills, Ikere",
    ],
  },
  {
    state: "Enugu",
    capital: "Enugu",
    slogan: "Coal City State",
    senatorial_districts: ["Enugu North", "Enugu East", "Enugu West"],
    lgas: [
      "Aninri",
      "Awgu",
      "Enugu-East",
      "Enugu-North",
      "Enugu-South",
      "Ezeagu",
      "Igbo-Etiti",
      "Igbo-Eze-North",
      "Igbo-Eze-South",
      "Isi-Uzo",
      "Nkanu-East",
      "Nkanu-West",
      "Nsukka",
      "Nukanu East",
      "Oji-River",
      "Udenu",
      "Udi",
      "Uzo-Uwani",
    ],
    landmass: "7,161 km2 (2,765 sq mi)",
    population: "3,257,298",
    dialect: "Igbo",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "6.4500",
    longitude: "7.5000",
    website: "http://www.enugustate.gov.ng/",
    geo_politcal_zone: "South East",
    created_date: "27 August 1991",
    created_by: "General Ibrahim Babangida",
    past_governors: [
      "Okwesilieze Nwodo",
      "Chimaroke Nnamani",
      "Sullivan Chime",
      "Ifeanyi Ugwuanyi",
    ],
    borders: ["Abia", "Anambra", "Benue", "Ebonyi", "Kogi"],
    known_for: [
      "Enugu Rangers",
      "Enugu State University of Science and Technology",
      "University of Nigeria, Nsukka",
      "Enugu State University Teaching Hospital",
      "Enugu State Broadcasting Service",
      "Enugu State College of Education (Technical)",
    ],
  },
  {
    state: "Federal Capital Territory",
    capital: "Abuja",
    slogan: "Centre of Unity",
    senatorial_districts: ["Federal Capital Territory"],
    lgas: ["Abaji", "Abuja Municipal", "Gwagwalada", "Kuje", "Bwari", "Kwali"],
    landmass: "7,315 km2 (2,824 sq mi)",
    population: "1,405,201",
    dialect: "English",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "9.0833",
    longitude: "7.5333",
    website: "http://www.fct.gov.ng/",
    geo_politcal_zone: "North Central",
    created_date: "3 February 1976",
    created_by: "General Murtala Mohammed",
    borders: ["Kogi", "Nasarawa", "Niger", "Kaduna", "Kwara", "Plateau"],
    past_governors: [
      "John Jatau Kadiya",
      "Abdulkadir Balarabe Musa",
      "Mike Akhigbe",
      "Peter Yandaka Afunanya",
      "Jerry Useni",
      "Hamza Abdullahi",
      "Ibrahim Bonu",
      "Abba Gana",
      "Nasir El rufai",
      "Mohammed Bello",
    ],

    known_for: [
      "Zuma Rock, Madalla",
      "Millennium Park, Maitama",
      "Jabi lake, Jabi",
      "National Mosque, Central Business District",
      "Lower Usman Dam, Bwari",
    ],
  },
  {
    state: "Gombe",
    capital: "Gombe",
    slogan: "Jewel in the Savannah",
    senatorial_districts: ["Gombe Central", "Gombe North", "Gombe South"],
    lgas: [
      "Akko",
      "Balanga",
      "Billiri",
      "Dukku",
      "Funakaye",
      "Gombe",
      "Kaltungo",
      "Kwami",
      "Nafada/Bajoga",
      "Shomgom",
      "Yamaltu/Deba",
    ],
    landmass: "20,265 km2 (7,828 sq mi)",
    population: "2,353,879",
    dialect: "Hausa",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "10.2897",
    longitude: "11.1711",
    website: "http://www.gombestate.gov.ng/",
    geo_politcal_zone: "North East",
    created_date: "1 October 1996",
    created_by: "General Sani Abacha",
    borders: ["Adamawa", "Bauchi", "Borno", "Yobe", "Taraba"],
    past_governors: [
      "Abubakar Habu Hashidu",
      "Danjuma Goje",
      "Ibrahim Hassan Dankwambo",
      "Inuwa Yahaya",
    ],
    known_for: [
      "Tomb of Buba Yero, Gombe",
      "Kaltungo Slave Route, Kaltungo",
      "Bima Hill, Gombe",
      "Tangale Hill, Tangale",
    ],
  },
  {
    state: "Imo",
    capital: "Owerri",
    slogan: "Eastern Heartland",
    senatorial_districts: ["Imo East", "Imo North", "Imo West"],
    lgas: [
      "Aboh-Mbaise",
      "Ahiazu-Mbaise",
      "Dral-Esat",
      "Ehime-Mbano",
      "Ezeobodo",
      "Ezinihitte",
      "Ideato",
      "Ideato-North",
      "Ideato-South",
      "Ihitte/Uboma",
      "Ikeduru",
      "Isiala-Mbano",
      "Isu",
      "Mbaitoli",
      "Mbano",
      "Ngor-Okpala",
      "Njaba",
      "Nkwerre",
      "Nwangele",
      "Obowo",
      "Oguta",
      "Ohaji-Egbema",
      "Okigwe",
      "Onuimo",
      "Orlu",
      "Oro-West",
      "Orsu",
      "Oru-East",
      "Oru-West",
      "Owerri-Municipal",
      "Owerri-North",
      "Owerri-West",
      "Ugiri-Ike Ikeduru",
      "Ugiri-Ikedikeduru",
      "Unbano",
      "Zinihitte",
    ],
    landmass: "5,530 km2 (2,140 sq mi)",
    population: "4,833,171",
    dialect: "Igbo",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "5.4833",
    longitude: "7.0333",
    website: "http://www.imostate.gov.ng/",
    geo_politcal_zone: "South East",
    created_date: "3 February 1976",
    created_by: "Murtala Mohammed",
    borders: ["Abia", "Anambra", "Ebonyi", "Enugu", "Rivers"],
    past_governors: [
      "Sam Mbakwe",
      "Evan Enwerem",
      "Judas Iwu",
      "Achike Udenwa",
      "Ikedi Ohakim",
      "Rochas Okorocha",
      "Emeka Ihedioha",
    ],
    known_for: [
      "Oguta Lake, Oguta",
      "Owerri Amusement Park, Owerri",
      "Owerri Mall, Owerri",
      "Owerri Zoo, Owerri",
    ],
  },
  {
    state: "Jigawa",
    capital: "Dutse",
    slogan: "The New World",
    senatorial_districts: [
      "Jigawa North - West",
      "Jigawa North – East",
      "Jigawa South – West",
    ],
    lgas: [
      "Auyo",
      "Babura",
      "Biriniwa",
      "Birnin-Kudu",
      "Bosuwa",
      "Buji",
      "Dutse",
      "Gagarawa",
      "Garki",
      "Gumel",
      "Guri",
      "Gwaram",
      "Gwiwa",
      "Hadejia",
      "Jahun",
      "Kafin-Hausa",
      "Kaugama",
      "Kazaure",
      "Kirkasamma",
      "Maigatari",
      "Malam-Maduri",
      "Miga",
      "Ringim",
      "Roni",
      "Sule-Tankarkar",
      "Taura",
      "Yankwashi",
    ],
    landmass: "22,410 km2 (8,650 sq mi)",
    population: "4,348,649",
    dialect: "Hausa",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "12.7500",
    longitude: "9.9667",
    website: "http://www.jigawastate.gov.ng/",
    geo_politcal_zone: "North West",
    created_date: "27 August 1991",
    created_by: "Ibrahim Babangida",
    borders: ["Bauchi", "Kano", "Katsina", "Kano", "Yobe", "Zamfara"],
    past_governors: [
      "Ali Sa’ad Birnin-Kudu",
      "Ibrahim Saminu Turaki",
      "Samaila Bature Chamah",
      "Sami Anka",
      "Abubakar Maimalari",
      "Sule Lamido",
      "Ali Sa’ad Birnin-Kudu",
      "Muhammad Badaru Abubakar",
    ],
    known_for: [
      "Kazaure Emirate",
      "Kiyawa Emirate",
      "Birnin Kudu Rock Painting, Birnin Kudu",
      "Baturiya Wetlands and Bird Sanctuary, Baturiya",
      "Jigawa State Museum, Dutse",
    ],
  },
  {
    state: "Kebbi",
    capital: "Birnin Kebbi",
    slogan: "Land of Equity",
    senatorial_districts: ["Kebbi Central", "Kebbi North", "Kebbi South"],
    lgas: [
      "Aleiro",
      "Arewa-Dandi",
      "Argungu",
      "Augie",
      "Bagudo",
      "Birnin-Kebbi",
      "Bumza",
      "Dandi",
      "Danko",
      "Fakai",
      "Gwandu",
      "Jega",
      "Kalgo",
      "Koko-Besse",
      "Maiyama",
      "Ngaski",
      "Sakaba",
      "Shanga",
      "Suru",
      "Wasagu",
      "Yauri",
      "Zuru",
    ],
    landmass: "36,800 km2 (14,200 sq mi)",
    population: "3,238,628",
    dialect: "Hausa",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "12.4500",
    longitude: "4.1994",
    website: "http://www.kebbistate.gov.ng/",
    geo_politcal_zone: "North West",
    created_date: "27 August 1991",
    created_by: "Ibrahim Babangida",
    borders: ["Niger", "Zamfara", "Katsina", "Sokoto"],
    past_governors: [
      "Abubakar Musa",
      "Adamu Aliero",
      "Usman Saidu Nasamu Dakingari",
      "Abubakar Atiku Bagudu",
    ],
    known_for: [
      "Kanta National Museum, Argungun",
      "Argungu Fishing Festival, Argungu",
      "Kanta Museum, Argungu",
      "Girmache Shrine, Zuru",
      "Karishi Traditional Settlement, Karishi",
    ],
  },
  {
    state: "Kaduna",
    capital: "Kaduna",
    slogan: "Centre of Learning",
    senatorial_districts: ["Kaduna Central", "Kaduna North", "Kaduna South"],
    lgas: [
      "Birnin-Gwari",
      "Chikun",
      "Giwa",
      "Gwagwada",
      "Igabi",
      "Ikara",
      "Jaba",
      "Jema'A",
      "Kachia",
      "Kaduna-North",
      "Kagarko",
      "Kajuru",
      "Kaura",
      "Kauru",
      "Koka/Kawo",
      "Kubah",
      "Kudan",
      "Lere",
      "Makarfi",
      "Sabon-Gari",
      "Sanga",
      "Soba",
      "Tudun-Wada/Makera",
      "Zango-Kataf",
      "Zaria",
    ],
    landmass: "46,053 km2 (17,782 sq mi)",
    population: "6,066,562",
    dialect: "Hausa",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "10.5167",
    longitude: "7.4333",
    website: "http://www.kadunastate.gov.ng/",
    geo_politcal_zone: "North West",
    created_date: "27 May 1967",
    created_by: "Gen. Yakubu Gowon",
    borders: [
      "Kano",
      "Katsina",
      "Niger",
      "Plateau",
      "Nasarawa",
      "Benue",
      "Kogi",
      "Zamfara",
      "Sokoto",
      "Kebbi",
      "Bauchi",
      "Gombe",
    ],
    past_governors: ["Abdulkadir Balarabe Musa", "Lawal Kaita", "Umaru Dikko"],
    known_for: [
      "Kajuru Castle, Kajuru",
      "Kofar Gamji Park, Kaduna",
      "Kamuku National Park, Birnin Gwari",
      "Kaduna Museum, Kaduna",
      "Kajuru Castle, Kajuru",
      "Kofar Gamji Park, Kaduna",
      "Kamuku National Park, Birnin Gwari",
      "Kaduna Museum, Kaduna",
    ],
  },
  {
    state: "Kano",
    capital: "Kano",
    slogan: "Centre of Commerce",
    senatorial_districts: ["Kano South", "Kano Central", "Kano North"],
    lgas: [
      "Ajingi",
      "Albasu",
      "Bagwai",
      "Bebeji",
      "Bichi",
      "Bunkure",
      "Dala",
      "Dambatta",
      "Dawakin-Kudu",
      "Dawakin-Tofa",
      "Doguwa",
      "Fagge",
      "Gabasawa",
      "Garko",
      "Garun-Mallam",
      "Gaya",
      "Gezawa",
      "Gwale",
      "Gwarzo",
      "Kano-Municipal",
      "Karaye",
      "Kibiya",
      "Kiru",
      "Kumbotso",
      "Kunchi",
      "Kura",
      "Madobi",
      "Makoda",
      "Minjibir",
      "Nasarawa",
      "Rano",
      "Rimin-Gado",
      "Rogo",
      "Shanono",
      "Sumaila",
      "Takai",
      "Tarauni",
      "Tofa",
      "Tsanyawa",
      "Tudun-Wada",
      "Ungogo",
      "Warawa",
      "Wudil",
    ],
    landmass: "20,131 km2 (7,773 sq mi)",
    population: "9,401,288",
    dialect: "Hausa",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "11.9964",
    longitude: "8.5167",
    website: "http://www.kanostate.gov.ng/",
    geo_politcal_zone: "North West",
    created_date: "27 May 1967",
    created_by: "Gen. Yakubu Gowon",
    borders: [
      "Jigawa",
      "Katsina",
      "Bauchi",
      "Kaduna",
      "Katsina",
      "Zamfara",
      "Kebbi",
    ],
    past_governors: [
      "Abdullahi Umar Ganduje",
      "Rabiu Musa Kwankwaso",
      "Ibrahim Shekarau",
      "Rabiu Musa Kwankwaso",
    ],
    known_for: [
      "Kano City Walls",
      "Kano Emir's Palace",
      "Kurmi Market",
      "Kofar Mata Dyeing Pit",
      "Kano City Walls",
      "Kano Emir's Palace",
      "Kurmi Market",
      "Kofar Mata Dyeing Pit",
    ],
  },
  {
    state: "Kogi",
    capital: "Lokoja",
    slogan: "The Confluence State",
    senatorial_districts: ["Kogi Central", "Kogi East", "Kogi West"],
    lgas: [
      "Adavi",
      "Ajaokuta",
      "Ankpa",
      "Dekina",
      "Ibaji",
      "Idah",
      "Igalamela",
      "Ijumu",
      "Ikoyi-Ijumu",
      "Kabba/Bunu",
      "Kogi",
      "Lokoja",
      "Mopa-Muro-Mopi",
      "Obaji",
      "Ofu",
      "Ogori/Magongo",
      "Okehi",
      "Okene",
      "Olamaboro",
      "Omala",
      "Oru",
      "Oyi",
      "Yagba-East",
      "Yagba-West",
    ],
    landmass: "29,833 km2 (11,515 sq mi)",
    population: "3,314,043",
    dialect: "Igala",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "7.8000",
    longitude: "6.7333",
    website: "http://www.kogistate.gov.ng/",
    geo_politcal_zone: "North Central",
    created_date: "27 August 1991",
    created_by: "Gen. Ibrahim Babangida",
    borders: [
      "Nasarawa",
      "Benue",
      "Enugu",
      "Anambra",
      "Ekiti",
      "Ondo",
      "Edo",
      "Delta",
      "Kwara",
      "Niger",
      "FCT",
    ],
    past_governors: ["Yahaya Bello", "Idris Wada", "Ibrahim Idris"],
    known_for: [
      "Mount Patti",
      "Lord Lugard House",
      "Confluence of Rivers Niger and Benue",
      "Mount Patti",
      "Lord Lugard House",
      "Confluence of Rivers Niger and Benue",
    ],
  },
  {
    state: "Katsina",
    capital: "Katsina",
    slogan: "Home of Hospitality",
    senatorial_districts: ["Katsina Central", "Katsina North", "Katsina South"],
    lgas: [
      "Bakori",
      "Batagarawa",
      "Batsari",
      "Baure",
      "Bindawa",
      "Charanchi",
      "Dan-Musa",
      "Dandume",
      "Danji",
      "Daura",
      "Dutsi",
      "Dutsinma",
      "Faskari",
      "Funtua",
      "Ingawa",
      "Jibia",
      "Kafur",
      "Kaita",
      "Kankara",
      "Kankia",
      "Katsina",
      "Kurfi",
      "Kusada",
      "Mai-Adua",
      "Malumfashi",
      "Mani",
      "Mashi",
      "Matazu",
      "Musawa",
      "Rimi",
      "Sabuwa",
      "Safana",
      "Sandamu",
      "Zango",
    ],
    landmass: "24,192 km2 (9,338 sq mi)",
    population: "5,792,578",
    dialect: "Hausa",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "12.9908",
    longitude: "7.6000",
    website: "http://www.katsinastate.gov.ng/",
    geo_politcal_zone: "North West",
    created_date: "23 September 1987",
    created_by: "Gen. Ibrahim Babangida",
    borders: ["Jigawa", "Kano", "Kaduna", "Zamfara", "Maradi Region, Niger"],
    past_governors: [
      "Aminu Bello Masari",
      "Ibrahim Shema",
      "Umaru Musa Yar'Adua",
    ],
    known_for: ["Katsina Airport", "Katsina Airport"],
  },
  {
    state: "Kwara",
    capital: "Ilorin",
    slogan: "State of Harmony",
    senatorial_districts: ["Kwara Central", "Kwara North", "Kwara South"],
    lgas: [
      "Asa",
      "Baruten",
      "Edu",
      "Ekiti",
      "Ifelodun",
      "Ilorin south",
      "Ilorin west",
      "Ilorin east",
      "Irepodun",
      "Isin",
      "Kaiama",
      "Moro",
      "Offa",
      "Oke ero",
      "Oyun",
      "Pategi",
    ],
    landmass: "36,825 km2 (14,229 sq mi)",
    population: "2,365,353",
    dialect: "Yoruba",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "8.5000",
    longitude: "4.5500",
    website: "http://www.kwarastate.gov.ng/",
    geo_politcal_zone: "North Central",
    created_date: "27 May 1967",
    created_by: "Gen. Yakubu Gowon",
    borders: [
      "Benue",
      "Kogi",
      "Ekiti",
      "Ondo",
      "Osun",
      "Oyo",
      "Niger",
      "Kebbi",
    ],
    past_governors: [
      "Abdulfatah Ahmed",
      "Bukola Saraki",
      "Adamu Attah",
      "David Bamigboye",
    ],
    known_for: [
      "Kainji National Park",
      "Esie Museum",
      "Kainji National Park",
      "Esie Museum",
    ],
  },
  {
    state: "Lagos",
    capital: "Ikeja",
    slogan: "Centre of Excellence",
    senatorial_districts: ["Lagos West", "Lagos Central", "Lagos East"],
    lgas: [
      "Agege",
      "Ajeromi-Ifelodun",
      "Alimosho",
      "Amuwo-Odofin",
      "Apapa",
      "Badagry",
      "Epe",
      "Eti-Osa",
      "Ibeju-Lekki",
      "Ifako-Ijaiye",
      "Ikeja",
      "Ikorodu",
      "Kosofe",
      "Lagos-Island",
      "Lagos-Mainland",
      "Mushin",
      "Ojo",
      "Oshodi-Isolo",
      "Shomolu",
      "Somolu",
      "Surulere",
    ],
    landmass: "3,577 km2 (1,380 sq mi)",
    population: "9,013,534",
    dialect: "Yoruba",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "6.5244",
    longitude: "3.3792",
    website: "http://www.lagosstate.gov.ng/",
    geo_politcal_zone: "South West",
    created_date: "27 May 1967",
    created_by: "Gen. Yakubu Gowon",
    borders: ["Ogun", "Ondo"],
    past_governors: [
      "Akinwunmi Ambode",
      "Babatunde Fashola",
      "Bola Tinubu",
      "Lateef Jakande",
    ],
    known_for: [
      "National Museum Lagos",
      "National Arts Theatre",
      "National Museum Lagos",
      "National Arts Theatre",
    ],
  },
  {
    state: "Nassarawa",
    capital: "Lafia",
    slogan: "Home of Solid Minerals",
    senatorial_districts: [
      "Nassarawa South",
      "Nassarawa North",
      "Nassarawa West",
    ],
    lgas: [
      "Akwanga",
      "Awe",
      "Doma",
      "Karu",
      "Keana",
      "Keffi",
      "Kokona",
      "Lafia",
      "Nassawara",
      "Nassawara Eggon",
      "Obi",
      "Wambu",
    ],
    landmass: "27,117 km2 (10,469 sq mi)",
    population: "1,863,275",
    dialect: "Eggon",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "8.5000",
    longitude: "8.5000",
    website: "http://www.nasarawastate.gov.ng/",
    geo_politcal_zone: "North Central",
    created_date: "1 October 1996",
    created_by: "Gen. Sani Abacha",
    borders: ["Benue", "Kogi", "Plateau", "Taraba", "Kaduna", "Kano", "Bauchi"],
    past_governors: [
      "Umaru Tanko Al-Makura",
      "Aliyu Akwe Doma",
      "Abdullahi Adamu",
    ],
    known_for: ["Farin Ruwa Falls", "Keffi", "Farin Ruwa Falls", "Keffi"],
  },
  {
    state: "Niger",
    capital: "Minna",
    slogan: "The Power State",
    senatorial_districts: ["Niger East", "Niger North", "Niger South"],
    lgas: [
      "Agaie",
      "Agwara",
      "Bida",
      "Borgu",
      "Bosso",
      "Chanchaga",
      "Edati",
      "Gbako",
      "Gurara",
      "Katcha",
      "Kontagora",
      "Lapai",
      "Lavun",
      "Magama",
      "Mariga",
      "Mashegu",
      "Mokwa",
      "Muya",
      "Paikoro",
      "Rafi",
      "Rijau",
      "Shiroro",
      "Suleja",
      "Tafa",
      "Wushishi",
    ],
    landmass: "76,363 km2 (29,484 sq mi)",
    population: "3,950,249",
    dialect: "Nupe",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "9.6000",
    longitude: "6.5500",
    website: "http://www.nigerstate.gov.ng/",
    geo_politcal_zone: "North Central",
    created_date: "3 February 1976",
    created_by: "Gen. Murtala Mohammed",
    borders: [
      "Kebbi",
      "Zamfara",
      "Kaduna",
      "Kogi",
      "Kwara",
      "Kebbi",
      "Kwara",
      "Kaduna",
      "Kogi",
      "Zamfara",
    ],
    past_governors: [
      "Abubakar Sani Bello",
      "Mu'azu Babangida Aliyu",
      "Abdulkadir Kure",
    ],
    known_for: ["Gurara Falls", "Kainji Lake", "Gurara Falls", "Kainji Lake"],
  },
  {
    state: "Ogun",
    capital: "Abeokuta",
    slogan: "Gateway State",
    senatorial_districts: ["Ogun Central", "Ogun East", "Ogun West"],
    lgas: [
      "Abeokuta-North",
      "Abeokuta-South",
      "Ado-Odo/Ota",
      "Ewekoro",
      "Ifo",
      "Ijebu-East",
      "Ijebu-North",
      "Ijebu-North-East",
      "Ijebu-Ode",
      "Ikenne",
      "Imeko-Afon",
      "Ipokia",
      "Obafemi-Owode",
      "Odeda",
      "Odogbolu",
      "Ogun-Waterside",
      "Remo-North",
      "Sagamu",
      "Yewa North",
      "Yewa South",
    ],
    landmass: "16,762 km2 (6,473 sq mi)",
    population: "3,751,140",
    dialect: "Egba",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "7.1500",
    longitude: "3.3500",
    website: "http://www.ogunstate.gov.ng/",
    geo_politcal_zone: "South West",
    created_date: "3 February 1976",
    created_by: "Gen. Murtala Mohammed",
    borders: ["Ondo", "Oyo", "Osun", "Ondo", "Oyo", "Osun"],
    past_governors: ["Ibikunle Amosun", "Gbenga Daniel", "Olusegun Osoba"],
    known_for: ["Olumo Rock", "Owu Waterfalls", "Olumo Rock", "Owu Waterfalls"],
  },
  {
    state: "Ondo",
    capital: "Akure",
    slogan: "Sunshine State",
    senatorial_districts: ["Ondo Central", "Ondo North", "Ondo South"],
    lgas: [
      "Akoko",
      "Akoko-North",
      "Akoko-North-West",
      "Akoko-South",
      "Akoko-South-East",
      "Akure",
      "Akure-North",
      "Akure-South",
      "Ese-Odo",
      "Idanre",
      "Ifedore",
      "Igbisin",
      "Ikale",
      "Ilaje",
      "Ilaje-West",
      "Ile-Oluji-Okeigbo",
      "Irele",
      "Odigbo",
      "Oka Ak0Ko",
      "Okiti Pupa Ijuodo",
      "Okiti-Pupa",
      "Ondo",
      "Ondo West",
      "Ondo-East",
      "Ose",
      "Owo",
    ],
    landmass: "15,500 km2 (6,000 sq mi)",
    population: "3,441,024",
    dialect: "Akure",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "7.2500",
    longitude: "5.2000",
    website: "http://www.ondostate.gov.ng/",
    geo_politcal_zone: "South West",
    created_date: "3 February 1976",
    created_by: "Gen. Murtala Mohammed",
    borders: ["Ogun", "Osun", "Ekiti", "Ogun", "Osun", "Ekiti"],
    past_governors: [
      "Oluwarotimi Odunayo Akeredolu",
      "Olusegun Mimiko",
      "Olusegun Agagu",
    ],
    known_for: ["Idanre Hills", "Owo Museum", "Idanre Hills", "Owo Museum"],
  },
  {
    state: "Osun",
    senatorial_districts: ["Osun Central", "Osun East", "Osun West"],
    lgas: [
      "Atakumosa",
      "Atakumosa East",
      "Ayeda-Ade",
      "Ayedire",
      "Boluwaduro",
      "Boripe",
      "Ede",
      "Ede North",
      "Egbedore",
      "Ejigbo",
      "Ife",
      "Ife North",
      "Ife South",
      "Ife-Central",
      "Ife-East",
      "Ifelodun",
      "Ila",
      "Ilesa-East",
      "Ilesa-West",
      "Ilesha",
      "Ilesha West",
      "Irepodun",
      "Irewole",
      "Isokan",
      "Iwo",
      "Obokun",
      "Odo-Otin",
      "Ola Oluwa",
      "Olorunda",
      "Ori-Ade",
      "Orolu",
      "Osogbo",
    ],
    landmass: "9,251 km2 (3,572 sq mi)",
    population: "3,423,535",
    dialect: "Ife",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "7.2500",
    longitude: "5.2000",
    website: "http://www.osun.gov.ng/",
    geo_politcal_zone: "South West",
    created_date: "27 August 1991",
    created_by: "Gen. Ibrahim Babangida",
    borders: ["Oyo", "Ekiti", "Ondo"],
    past_governors: ["Rauf Aregbesola", "Olagunsoye Oyinlola", "Bisi Akande"],
    known_for: ["Osun-Osogbo", "Osun-Osogbo"],
  },
  {
    state: "Oyo",
    capital: "Ibadan",
    slogan: "Pace Setter",
    senatorial_districts: ["Oyo Central", "Oyo North", "Oyo South"],
    lgas: [
      "Afijio",
      "Akinyele",
      "Atiba",
      "Atisbo",
      "Egbeda",
      "Ibadan-Central",
      "Ibadan-North-East",
      "Ibadan-North-West",
      "Ibadan-South-East",
      "Ibadan-South-West",
      "Ibarapa-Central",
      "Ibarapa-East",
      "Ibarapa-North",
      "Ido",
      "Irepo",
      "Iseyin",
      "Itseiwaju",
      "Iwajowa",
      "Kajola",
      "Lagelu",
      "Odo-Oluwa",
      "Ogbomosho-North",
      "Ogbomosho-South",
      "Olorunsogo",
      "Oluyole",
      "Ona-Ara",
      "Orelope",
      "Ori-Ire",
      "Oyo-East",
      "Oyo-West",
      "Saki-East",
      "Saki-West",
      "Surulere",
    ],
    landmass: "28,454 km2 (10,986 sq mi)",
    population: "5,580,894",
    dialect: "Ibadan",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "7.2500",
    longitude: "5.2000",
    website: "http://www.oyostate.gov.ng/",
    geo_politcal_zone: "South West",
    created_date: "3 February 1976",
    created_by: "Gen. Murtala Mohammed",
    borders: ["Ogun", "Osun", "Kwara", "Ogun", "Osun", "Kwara"],
    past_governors: [
      "Abiola Ajimobi",
      "Christopher Alao-Akala",
      "Rashidi Ladoja",
      "Lam Adesina",
    ],
    known_for: ["Ibadan", "Ibadan"],
  },
  {
    state: "Plateau",
    capital: "Jos",
    slogan: "Home of Peace and Tourism",
    senatorial_districts: ["Plateau Central", "Plateau North", "Plateau South"],
    lgas: [
      "Barkin-Ladi",
      "Bassa",
      "Bokkos",
      "Jos-East",
      "Jos-North",
      "Jos-South",
      "Kanam",
      "Kanke",
      "Langtang-North",
      "Langtang-South",
      "Mangu",
      "Mikang",
      "Pankshin",
      "Quan'Anpan",
      "Riyom",
      "Shendam",
      "Wase",
    ],
    landmass: "26,899 km2 (10,386 sq mi)",
    population: "3,178,712",
    dialect: "Jos",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "7.2500",
    longitude: "5.2000",
    website: "http://www.plateaustate.gov.ng/",
    geo_politcal_zone: "North Central",
    created_date: "3 February 1976",
    created_by: "Gen. Murtala Mohammed",
    borders: [
      "Bauchi",
      "Kaduna",
      "Nasarawa",
      "Taraba",
      "Bauchi",
      "Kaduna",
      "Nasarawa",
      "Taraba",
    ],
    past_governors: [
      "Simon Lalong",
      "Jonah Jang",
      "Joshua Dariye",
      "Fidelis Tapgun",
    ],
    known_for: ["Jos", "Jos"],
  },
  {
    state: "Rivers",
    capital: "Port Harcourt",
    slogan: "Treasure Base of the Nation",
    senatorial_districts: ["Rivers East", "Rivers South East", "Rivers West"],
    lgas: [
      "Aboa/Odual",
      "Ahoada-East",
      "Ahoada-West",
      "Akukutoru",
      "Andoni",
      "Asari-Toru",
      "Bonny",
      "Buguma",
      "Degema",
      "Eleme",
      "Elfane",
      "Emuoha",
      "Etche",
      "Gokana",
      "Ikwerre",
      "Khana",
      "Obia/Akpor",
      "Ogba-Egbema-Ndoni",
      "Ogba/Egbema/Ndoni",
      "Ogu/Bolo",
      "Okirika",
      "Omuma",
      "Opobo/Nkoro",
      "Oyigbo",
      "Port-Harcourt",
      "Tai",
    ],
    landmass: "11,077 km2 (4,276 sq mi)",
    population: "5,198,716",
    dialect: "Port Harcourt",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "7.2500",
    longitude: "5.2000",
    website: "http://www.riversstate.gov.ng/",
    geo_politcal_zone: "South South",
    created_date: "27 May 1967",
    created_by: "Gen. Yakubu Gowon",
    borders: [
      "Abia",
      "Akwa Ibom",
      "Bayelsa",
      "Imo",
      "Abia",
      "Akwa Ibom",
      "Bayelsa",
      "Imo",
    ],
    past_governors: [
      "Nyesom Wike",
      "Rotimi Amaechi",
      "Peter Odili",
      "Rufus Ada-George",
    ],
    known_for: ["Port Harcourt", "Port Harcourt"],
  },
  {
    state: "Sokoto",
    capital: "Sokoto",
    slogan: "Seat of the Caliphate",
    senatorial_districts: ["Sokoto East", "Sokoto North", "Sokoto South"],
    lgas: [
      "Binji",
      "Bodinga",
      "Dange-Shuni",
      "Gada",
      "Goronyo",
      "Gudu",
      "Gwadabawa",
      "Illela",
      "Kebbe",
      "Kware",
      "Raba",
      "Sabo-Birni",
      "Shagari",
      "Silame",
      "Sokoto-North",
      "Sokoto-South",
      "Tambuwal",
      "Tangaza",
      "Tureta",
      "Wamakko",
      "Wurno",
      "Yabo",
    ],
    landmass: "25,973 km2 (10,034 sq mi)",
    population: "4,220,000",
    dialect: "Sokoto",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "13.0667",
    longitude: "5.2333",
    website: "http://www.sokotostate.gov.ng/",
    geo_politcal_zone: "North West",
    created_date: "3 February 1976",
    created_by: "Gen. Murtala Mohammed",
    borders: ["Kebbi", "Zamfara", "Kebbi", "Zamfara"],
    past_governors: [
      "Aminu Tambuwal",
      "Aliyu Magatakarda Wamakko",
      "Attahiru Bafarawa",
      "Garba Nadama",
    ],
    known_for: ["Sokoto", "Sokoto"],
  },
  {
    state: "Taraba",
    capital: "Jalingo",
    slogan: "Nature's Gift to the Nation",
    senatorial_districts: ["Taraba Central", "Taraba North", "Taraba South"],
    lgas: [
      "Ardo-Kola",
      "Bali",
      "Donga",
      "Gashaka",
      "Gassol",
      "Ibi",
      "Jalingo",
      "Karim-Lamido",
      "Kurmi",
      "Lau",
      "Oilingo",
      "Sardauna",
      "Takum",
      "Ussa",
      "Wukari",
      "Yorro",
      "Zing",
    ],
    landmass: "54,473 km2 (21,044 sq mi)",
    population: "2,300,736",
    dialect: "Taraba",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "8.9167",
    longitude: "11.3667",
    website: "http://www.tarabastate.gov.ng/",
    geo_politcal_zone: "North East",
    created_date: "27 August 1991",
    created_by: "Gen. Ibrahim Babangida",
    borders: ["Bauchi", "Gombe", "Plateau", "Cameroon"],
    past_governors: [
      "Darius Ishaku",
      "Garba Umar",
      "Danbaba Danfulani Suntai",
      "Jolly Nyame",
    ],
    known_for: ["Jalingo", "Jalingo"],
  },
  {
    state: "Yobe",
    capital: "Damaturu",
    slogan: "Pride of the Sahel",
    senatorial_districts: ["Yobe East", "Yobe North", "Yobe South"],
    lgas: [
      "Bade",
      "Borsali",
      "Damaturu",
      "Fika",
      "Fune",
      "Geidam",
      "Gogaram",
      "Gujba",
      "Gulani",
      "Jakusko",
      "Karasuwa",
      "Machina",
      "Nangere",
      "Nguru",
      "Potiskum",
      "Tarmua",
      "Yunusari",
      "Yusufari",
    ],
    landmass: "45,502 km2 (17,575 sq mi)",
    population: "2,321,591",
    dialect: "Yobe",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "12.0000",
    longitude: "11.5000",
    website: "http://www.yobestate.gov.ng/",
    geo_politcal_zone: "North East",
    created_date: "27 August 1991",
    created_by: "Gen. Ibrahim Babangida",
    borders: ["Bauchi", "Borno", "Gombe", "Jigawa"],
    past_governors: [
      "Mai Mala Buni",
      "Ibrahim Geidam",
      "Bukar Abba Ibrahim",
      "Lawan Gana Karasuwa",
    ],
    known_for: ["Damaturu", "Damaturu"],
  },
  {
    state: "Zamfara",
    capital: "Gusau",
    slogan: "Farming is Our Pride",
    senatorial_districts: ["Zamfara Central", "Zamfara North", "Zamfara West"],
    lgas: [
      "Anka",
      "Bakura",
      "Bukkuyum",
      "Bungudu",
      "Gumi",
      "Gusau",
      "Isa",
      "Kaura-Namoda",
      "Kiyawa",
      "Maradun",
      "Maru",
      "Shinkafi",
      "Talata-Mafara",
      "Tsafe",
      "Zurmi",
    ],
    landmass: "39,762 km2 (15,352 sq mi)",
    population: "3,278,873",
    dialect: "Zamfara",
    map: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Nigeria_Akwa_Ibom_state_map.png/250px-Nigeria_Akwa_Ibom_state_map.png",
    latitude: "12.1700",
    longitude: "6.6600",
    website: "http://www.zamfarastate.gov.ng/",
    geo_politcal_zone: "North West",
    created_date: "1 October 1996",
    created_by: "Gen. Sani Abacha",
    borders: ["Kaduna", "Katsina", "Kebbi", "Sokoto"],
    past_governors: [
      "Bello Matawalle",
      "Abdulaziz Yari Abubakar",
      "Mahmud Shinkafi",
      "Ahmed Sani Yerima",
    ],
    known_for: ["Gusau", "Gusau"],
  },
];
